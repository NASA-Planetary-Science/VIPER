...
;   CENTEQ:      If set, the field line will be traced until at the centrifugal equator
;                  Added by Logan on 10/12/16
;
; OUTPUTS:
; 
...
;   
;     fp1 = footpoint(5, 90, 0, 'VIP4', [0.0001, 0.01], /BVARY, /LIN, /FULL)
;     fp2 = footpoint(5, 90, 0, 'VIP4', [0.0001, 0.01], /BVARY, /LIN, /FULL, /REV)
;     
...
function footpoint_logan, rr, theta, phi, model, inStepSize, time, $
...
; Logan - Check that code is stepping in the right direction to find Centrifugal equator
DELVAR, WRONG_DIR
WRONG_DIR_LABEL:
IF ISA(WRONG_DIR) THEN BEGIN
  IF revDir EQ 0 THEN revDir = 1 ELSE revDir = 0
ENDIF
...
;---Calculate initial distance from spin pole. Used for the CentEq keyword later. Set up counter (Logan 10/12/16)---
r_cyl_old = nextR * SIN(NextTheta*DEG2RAD)
nct = 0
...
WHILE cont DO BEGIN
  ...
  ;--Save old values before stepping for CentEq calculation (Logan 10/12/16)--
  ; Still need to work out if in degrees or radians
  phi_old   = nextPhi
  z_Old     = nextR * COS(nextTheta*DEG2RAD)
  ...
  ; Check for the centrifugal equator
  ; Logan 10/12/16
  IF KEYWORD_SET(CENTEQ) THEN BEGIN
    ; Still need to work out if in degrees or rad
    r_cyl_new = NextR * SIN(NextTheta*DEG2RAD)
    nct = nct + 1
    ; Check if at centrifugal equator
    ; If next step is at a lower radius than previous step, then the previous should have been the centrifugal equator
    IF r_cyl_new LT r_cyl_old THEN BEGIN
      ; Check that it wasn't on first iteration so that it is stepping in the right direction
      IF nct EQ 1 THEN BEGIN
        IF NOT ISA(WRONG_DIR) THEN BEGIN
          PRINT, 'Found maximum R on first iteration. Reversing step direction.'
          WRONG_DIR = 1
          GOTO, WRONG_DIR_LABEL
        ENDIF ELSE BEGIN
          PRINT, 'Found maximum R on first iteration again. Location is already the centrifugal equator.'
        ENDELSE
      ENDIF
      ; Create final elements and break from while loop
      r_cyl_fin   = r_cyl_old
      phi_cyl_fin = phi_old
      z_cyl_fin   = z_old
    ...
    ; If the new radius is still greater than the old one, continue stepping until at the centrifugal equator 
    IF r_cyl_new GT r_cyl_old THEN BEGIN
      r_cyl_old = r_cyl_new
    ENDIF
  ENDIF
  ...
  ENDIF ELSE IF finalR eq 0 THEN BEGIN
    IF KEYWORD_SET(CentEq) THEN BEGIN ; LOGAN 10/12/16 - Just skips the jovSurface call for the CentEq calculation
    ENDIF ELSE cont = nextR gt jovSurface(nextTheta)
...
IF CentEQ THEN BEGIN
  return, [r_cyl_fin, phi_cyl_fin, z_cyl_fin]
ENDIF
