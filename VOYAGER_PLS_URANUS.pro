FUNCTION VOYAGER_PLS_URANUS, Structure, POD, cupss, EDP, EPL, MAKE_PLOT=MAKE_PLOT, UNCERTAINTIES=UNCERTAINTIES, Structure_Unc=Structure_Unc, $
  ...
  ; CALC_FITTING_RESIDUALS - Calculated the residuals between the fitting and the raw data
  ...
  ; Adapted for Uranus by: George Xystouris
  
  ; Have Uranian physical characteristics - it will make your life easier
  r_Uranus = 25559d     ; Radius of Uranus [km]
  u_Rotation = -2.586d  ; Rotation speed of Uranus [km/s]
  ...
  FORWARD_FUNCTION Generate_PLS_Current_Uranus
  FORWARD_FUNCTION ADDITIONAL_PLS_FUNCTIONS
  FORWARD_FUNCTION CONRD
  FORWARD_FUNCTION Extrapolate_to_Equator  ;it's not working for Uranus at the moment
...
; ERROR DEFINITION:
;
; a) Error used for Jupiter
;  PLS_Error = SQRT(1d5 + 0.25d-4*PLS_Data*PLS_Data) ; Error calculated based on fitting residuals - more details in Voyager-PLS-Error-Analysis.pdf - this was OK for Jupiter, but for Uranus is HUGE
;
; b) Theoretical error "based on a past memo" 
; PLS_Error = SQRT(1.118d*1d-4*PLS_Data*PLS_Data)
;
; c) Jupiter methodology with 1% of measurement, and background variables based on Memo 178
;  IF Structure.L_or_M_Mode EQ 1 THEN PLS_Error = SQRT( 80d^2 + (1d-2*PLS_Data)^2 ) ELSE BEGIN
;    CASE Structure.Short_or_Long OF
;      1 : PLS_Error = SQRT( 9.49d^2 + (1d-2*PLS_Data)^2 )
;      0 : PLS_Error = SQRT( 20d^2 + (1d-2*PLS_Data)^2 )
;    ENDCASE
;  ENDELSE
;  d) Similar to c with 0.5% of measurement
;  IF Structure.L_or_M_Mode EQ 1 THEN PLS_Error = SQRT( 80d^2 + ((1d-2*PLS_Data)/2)^2 ) ELSE BEGIN
;    CASE Structure.Short_or_Long OF
;      1 : PLS_Error = SQRT( 9.49d^2 + ((1d-2*PLS_Data)/2)^2 )
;      0 : PLS_Error = SQRT( 20d^2 + ((1d-2*PLS_Data)/2)^2 )
;    ENDCASE
;  ENDELSE
;  e) Only background
;  IF Structure.L_or_M_Mode EQ 1 THEN PLS_Error = SQRT( 80d^2 ) ELSE BEGIN
;    CASE Structure.Short_or_Long OF
;      1 : PLS_Error = SQRT( 9.49d^2 )
;      0 : PLS_Error = SQRT( 20d^2 )
;    ENDCASE
;  ENDELSE
;  f) Background + Measurement uncertainty: Poisson (square root of measurement). Background error is defined
;   as the thermal noise of the electronics. Here we are using the thermal noise as given in Memo 178. Even 
;   though it's not a pre-Uranus calibration (unsure whether there is one), it is very close to the instrument
;   sensitivity during the Uranus flyby. (GX: Reasoning: PLS was blasted with radiation at Jupiter, while at 
;   Saturn and Uranus the radiation was not that strong. Therefore after the Jupiter flyby can be considered 
;   that PLS was not affected much by any radiation, i.e. energetic particles. Also Neptune is the closest we
;   have for Uranus, and it's much more representing rather than taking the pre-launch calibration sensitivity.)
  IF Structure.L_or_M_Mode EQ 1 THEN PLS_Error = SQRT( 37.96d^2 + PLS_Data ) ELSE BEGIN ; 37.96 = 4 * 9.49 - as per Memo 178
    CASE Structure.Short_or_Long OF
  ...
  IF KEYWORD_SET(CEq) THEN BEGIN
    ; Get location of S/C in URANUS_FIT coordinates            
    COMMON VGR2Data_URANUS
    SSEDR_Data = VGR2SSEDR_URANUS

    ; Time in SSEDR file                                                                                             THEY ARE GETTING THE CLOSEST TIMES BETWEEN THE TWO (S/C & PLS)
    SSEDR_Time = SSEDR_Data[1,*] + (SSEDR_Data[2,*] + SSEDR_Data[3,*]/60d + (SSEDR_Data[4,*] + (SSEDR_Data[5,*]/1000d) )/3600d)/24d
    ...
    xSC_URANUS_FIT = SSEDR_Data[6,SSEDR_ind] ; URANUS_FIT cartesian coordinates - see documentation or paper
    ySC_URANUS_FIT = SSEDR_Data[7,SSEDR_ind]
    zSC_URANUS_FIT = SSEDR_Data[8,SSEDR_ind]

    Position_URANUS_FIT = [xSC_URANUS_FIT, ySC_URANUS_FIT, zSC_URANUS_FIT] 
    New_Structure = Extrapolate_To_Equator(Structure, Position_URANUS_FIT, r_Uranus, /ISOTROPIC)
...
; THIS WAS THE ORIGINAL, AS WAS USED FOR JUPITER.
;  
;  IF Structure.L_or_M_Mode EQ -1 THEN BEGIN
;    IF KEYWORD_SET(UNCERTAINTIES) THEN PLS_electron_current = PLS_Data ; you're doing this to keep the original current values that will be used in the PLS_uncertainties.pro [GX]  
;  ; CREATE GRID POTENTIALS
;  phij = MAKE_ARRAY(29)
;  FOR i = 0, 28 DO BEGIN
;    IF i LE 16 THEN BEGIN
;      phij(i) = 60.*10.^(double(i)/32.) - 50.
...
;
;  ; VELOCITY POTENTIAL
;  ECHRGE = 1.6d-19                        ; Electron charge in Coulombs
;  me = 9.11d-31                           ; Electron mass in kg
;  Aeff = 100/1d4                          ; Area of sensor (cm^2) converted to m^2
;  TN = 0.56                               ; normal transmision of sensor (see section 2 of )
;  vj = SQRT(2. * ECHRGE * phij / (me))    ; electron speed for each grid to overcome the potential and hit the detector
;  ; Create appropriately sized arrays
;  dvj = MAKE_ARRAY(28)                    ;
;  vjavg = dvj                             ; average speed array
;  alphaj = dvj                            ;
;  ; For each step of the grid we calculate:
;  FOR loopi = 0, 27 DO BEGIN
;    dvj(loopi) = vj(loopi+1) - vj(loopi)          ; electrons speed difference from next grid
;    vjavg(loopi) = 0.5d*(vj(loopi+1) + vj(loopi)) ; average electron speed for each grid
...
;  PLS_DATA = PLS_DATA*1d-15/alphaj
;  PLS_DATA_Transp[3,*] = PLS_DATA_Transp[3,*]*1d-15/alphaj
;  PLS_ERROR = PLS_ERROR*1d-15/alphaj ; 1d-15 A to fA, but current is already in fA?
;  ;STOP
;
;ENDIF
...
;------------------ GX CHANGE FROM HERE
    ...
    xSC_URANUS_FIT = SSEDR_Data[6,SSEDR_ind] ; URANUS_FIT cartesian coordinates
    ySC_URANUS_FIT = SSEDR_Data[7,SSEDR_ind]
    zSC_URANUS_FIT = SSEDR_Data[8,SSEDR_ind]
    r_RU = SQRT(xSC_URANUS_FIT*xSC_URANUS_FIT+ySC_URANUS_FIT*ySC_URANUS_FIT+zSC_URANUS_FIT*zSC_URANUS_FIT)/r_Uranus
      
    xSC_RU = xSC_URANUS_FIT/r_Uranus
    ySC_RU = ySC_URANUS_FIT/r_Uranus
    zSC_RU = zSC_URANUS_FIT/r_Uranus
    SC_lat = asin(zSC_RU/r_RU)
    SC_lon = 180./!Pi*ATAN(ySC_RU,xSC_RU)
  
    ; Calculate L-Shell              ; GX: We don't care about this right now for Uranus 
...
;------------------ TO HERE
  ...
  IF Structure.Fit EQ 1 THEN BEGIN
   ...
   ENDELSE
    ...
    IF TOTAL(STRCMP(TAG_NAMES(Structure),'Short_or_Long', /FOLD_CASE))  EQ 1  THEN BEGIN
      IF Structure.L_or_M_Mode GE 0 THEN BEGIN
        IF Structure.L_or_M_Mode EQ 1 THEN Current_Threshold = 30.549d ELSE BEGIN
    ...
    ; If we have electrons, we are re-defining the threshold, as, above, it was defined based on the RDF
  ...
  ; --------------------------------------
     ...
     ; Logan 2025_07_15 - BESTNORM = 'the value of the summed squared residuals for the returned parameter values'
     IF Structure.L_or_M_Mode NE -1 THEN BEGIN     ; IONS
       Best_P = MPFITFUN('Fit_Parameters_Uranus', FLOAT(MAKE_ARRAY(4*n_channels, /DOUBLE)), FLOAT(PLS_Data_Fit), $
  ...
  ; --------------------------------------
    ...
    IF KEYWORD_SET(UNCERTAINTIES) THEN BEGIN
      ...
      Model_Name = 'Generate_PLS_Current_Uranus'
      ;
...
;        find the length of the PLS_Unc -> divide by 2 -> write variable in species
  ...
  IF KEYWORD_SET(CALC_FITTING_RESIDUALS) THEN BEGIN
      ...
      PLS_Species_Currents = Generate_PLS_Current_Uranus(Structure)
  ...
  IF KEYWORD_SET(MAKE_PLOT) THEN BEGIN
    IF Structure.L_or_M_Mode NE -1 THEN BEGIN ; Plotting procedure for ions in M and L-modes
      ...
      PLS_Currents = Generate_PLS_Current_Uranus(Structure)
      ...
      Plasma_Properties = Generate_PLS_Current_Uranus(Structure, /RETURN_PLASMA_PROPERTIES) ; this returns ONLY the plasma properties
      Masses       = Plasma_Properties[0,*]
      Charges      = Plasma_Properties[1,*]
      Densities    = Plasma_Properties[2,*]
      Temperatures = Plasma_Properties[3,*]
      
      ; COLOURS (HARDCODED). If we use two species of H then we have two colours
      ; (one for each species), otherwise we only have one (given that we are using a single species of H).
      IF N_ELEMENTS(Masses) EQ 2 THEN colors = ['sky blue','purple'] ELSE colors = MAKE_ARRAY(1, N_ELEMENTS(Masses), /STRING, VALUE = 'sky blue')
...
;      FOR k = 0, 2 DO q=text((1+k)*sep*2d + 0.15,0.12, string(dum_names[k], format='(F0.2)')) 
      ...
      ; Use Generate PLS Currents to get the plasma properties  
      Plasma_Properties = Generate_PLS_Current_Uranus(Structure, /RETURN_PLASMA_PROPERTIES)
      Masses       = Plasma_Properties[0,*]
      Charges      = Plasma_Properties[1,*]
      Densities    = Plasma_Properties[2,*]
      Temperatures = Plasma_Properties[3,*]
      ...
      ; could be up to 8 different species. For Uranus though, since we are only using two species, the colour was set to black for both of them. [GX]
      ...
      density_format[*,0] = '(F5.2)' & temperature_format[*,0] = '(F5.1)'
      IF N_ELEMENTS(masses) GE 2 THEN BEGIN ; this is specifically for the hot component
         density_format[1,0] = '(F4.2)' & temperature_format[1,0] = '(I 6)'
      ENDIF
      
      
      IF KEYWORD_SET(UNCERTAINTIES) THEN BEGIN
        FOR k = 0, N_ELEMENTS(Masses)-1 DO BEGIN
          v = TEXT((1+k)*seperation+.047,0.09, STRCOMPRESS(STRING(UINT(Masses(k)))+','+STRING(UINT(Charges(k)))), color='Black', /norm)
          v = TEXT((1+k)*seperation+.05,0.06, STRCOMPRESS(STRING(Densities[k],format = density_format[k]) + ' $\pm$ ' + STRING(Densities_Unc[k],format = density_format[k])), /norm)
          v = TEXT((1+k)*seperation+.05,0.03, STRCOMPRESS(STRING(Temperatures[k], format = temperature_format[k])+ ' $\pm$ ' + STRING(Temperatures_Unc(k),format = temperature_format[k])), /norm)
        ENDFOR
      ENDIF ELSE BEGIN
        FOR k = 0, N_ELEMENTS(Masses)-1 DO BEGIN
          v = TEXT((1+k)*seperation+.047,0.09, STRCOMPRESS(STRING(UINT(Masses(k)))+','+STRING(UINT(Charges(k)))), color='Black', /norm)
          v = TEXT((1+k)*seperation+.05,0.06, STRING(Densities[k],format = density_format[k]), /norm)
          v = TEXT((1+k)*seperation+.05,0.03, STRING(Temperatures[k], format = temperature_format[k]), /norm)
        ENDFOR
      ENDELSE
...
;      ; This part uses the same variables as have been calculated above, during the data fitting process
;     
;      ; The phase Space density (REDUCED DISTRIBUTION FUNCTION EQ. 23 IN SITTLER MEMO)
;      ; GRID POTENTIALS
;      phij = MAKE_ARRAY(29)
;      FOR i = 0, 28 DO BEGIN
;        IF i LE 16 THEN BEGIN
...
;        alphaj(loopi) = ECHRGE * Aeff  * TN * !Pi * vjavg(loopi) * vjavg(loopi) *vjavg(loopi) * dvj(loopi)
;      ENDFOR
;      
;      
;      ; FLUX CONVERSION
...
;      stop
      ...
      ; GENERATE FINAL RESULT OF MODEL
      PLS_Currents = Generate_PLS_Current_Uranus(Structure) ; in RDF for A
      ...
      FOR jloop = 0, n_species-1 DO BEGIN
        ...
        ; 3rd column:       electron flux (units?)
        ; 4th column:       electron current in fA
        PLS_Currents(0,jloop,*) = PLS_Currents(3,jloop,*) * 1d15; LOGAN 2025_06_11 - Based on line 926 of Voyager_PLS_Jupiter, should this be multiplied by 1d15 if I'm not mistaken
      ...
      ; Color titles for the plots
      colors = ['deep pink','sky blue','purple']
...
;        ALIGNMENT = .5, vertical_ALIGNMENT = .5, /normal)
      ...
      ; PLOT
      FOR k = 0, N_ELEMENTS(Masses)-1 DO BEGIN
        v = TEXT( (1+k)*seperation+ .05,0.12, STRCOMPRESS( STRING(UINT(Masses(k))) + ', -' + STRTRIM(UINT(Charges(k)),2)), /norm)
        IF KEYWORD_SET(UNCERTAINTIES) THEN BEGIN
          v = TEXT((1+k)*seperation+ .05,0.09, STRCOMPRESS(STRING(Densities[k],format = density_format[k]) + ' $\pm$ ' + STRING(Densities_Unc[k], format = density_format[k])), /norm)
          v = TEXT((1+k)*seperation+ .05,0.06, STRCOMPRESS(STRING(Temperatures[k], format = temperature_format[k]) + ' $\pm$ ' + STRING(Temperatures_Unc[k], format = temperature_format[k])), /norm)
          IF Structure.Response EQ 1 THEN v = TEXT((1+k)*seperation+ .05,0.03, STRING(Kappatext[k], format='(F0.2)'), /norm)
        ENDIF ELSE BEGIN
          v = TEXT((1+k)*seperation+ .05,0.09, STRING(Densities[k],format = density_format[k]), /norm)
          v = TEXT((1+k)*seperation+ .05,0.06, STRING(Temperatures[k], format = temperature_format[k]), /norm)
          IF Structure.Response EQ 1 THEN v = TEXT((1+k)*seperation+ .05,0.03, STRING(Kappatext[k], format='(F0.2)'), /norm)
        ENDELSE
        ;STOP
      ENDFOR
  ...
  IF EDP EQ 1 THEN BEGIN
    ...
    zSC_RU = zSC_URANUS_FIT/r_Uranus
    SC_lat = asin(zSC_RU/r_RU)
    SC_lon = asin(ySC_RU/(r_RU*cos(SC_lat)))
...
FUNCTION FIT_PARAMETERS_URANUS, X, P, err=err
  ...
  FORWARD_FUNCTION Generate_PLS_Current_Uranus
  Total_Current = Generate_PLS_Current_Uranus(Structure, /RETURN_TOTAL_CURRENT, /SET_FLOAT)
  
  ; Return Total_Current to MPFIT so it can find best fit parameters
  RETURN, Total_Current
END

FUNCTION Generate_PLS_Current_Uranus, Structure, RETURN_TOTAL_CURRENT=RETURN_TOTAL_CURRENT, SET_FLOAT=SET_FLOAT, $
  ...
  ; Determine Spacecraft (for Uranus is only Voyager 2) and pull appropriate SSEDR information for trajectory and location
...
FUNCTION GENERATE_ELECTRON_FLUXES_URANUS, DOY, OUTPUTNAME
...
;------------------ GX CHANGE FROM HERE
 ...
 IF Structure.Spacecraft EQ 2 THEN BEGIN
  ...
  FOR k = 0, nel-1 DO BEGIN
    ...
    ; Calculate radial distance of S/C from planet core [r_Uranus]
    R[k] = SQRT(xSC_S3*xSC_S3+ySC_S3*ySC_S3+zSC_S3*zSC_S3)/r_Uranus
  ENDFOR
;------------------ TO HERE
...
FUNCTION read_file_Uranus, Data_Filename, CHANGE_DIRECTORY=CHANGE_DIRECTORY
  ...
  WHILE ~EOF(lun) DO BEGIN
    ...
    ;                               LS    L-mode Short
    ;                               LL    L-mode Long
    ;                               ML    M-mode Long (there is not MS for Uranus)
    ;                               
...
; GX: FOR THE NEXT TWO FUNCTIONS: NOT NEEDING THEM FOR URANUS FOR NOW. THEY WORK FOR JUPITER
; 
...
;
;
FUNCTION Extrapolate_To_Equator, Structure, SC_Position, r_planet, ISOTROPIC=ISOTROPIC, LATDIST=LATDIST  ; GX: UNSURE IF NEEDED FOR URANUS
  ...
  ; SC_Position   - Location of the spacecraft in the coordination system we need (SIII for J, URANUS_FIT for U etc.)
...
;                       we are not using it (at least for now) for Uranus
;------------------   
